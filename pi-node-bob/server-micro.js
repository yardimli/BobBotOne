...
function startSerialControl() {
  ...
  startSerialControl_timeout = setTimeout(function () {
    ...
    serial_tunnel = child_process.fork('./raspberry-pi-micro-bob.js', [], {silent: true});
    ...
    serial_tunnel.stdout.on('data', function (data) {
      ...
      if (strdata.indexOf("reset_me") !== -1) {
        console.log("self killing serial/server control now. " + Bob_Settings.ServerSubDomain);
        log_to_file("self killing serial/server control now. " + Bob_Settings.ServerSubDomain);
